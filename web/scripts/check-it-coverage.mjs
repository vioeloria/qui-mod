...
const itRoot = path.join(localesRoot, "it")
...
function checkMissingKeys(enFlat, itFlat, namespace) {
  const errors = []

  // Build set of en plural bases that have _one/_other pairs (v4 CLDR style).
  // For these, it only needs _other -- _one is not required.
  ...
  for (const [key, value] of enFlat) {
    // Skip _one keys for v4 plural pairs -- it collapses to _other only.
...
function checkExtraKeys(enFlat, itFlat, namespace) {
  const errors = []

  for (const key of itFlat.keys()) {
    if (!enFlat.has(key)) {
      // If it has _other and en has _one/_other pair, that is fine.
...
function checkInterpolation(enFlat, itFlat, namespace) {
  const errors = []

  for (const [key, enValue] of enFlat) {
    const itValue = itFlat.get(key)
    if (!itValue) continue

    const enVars = extractInterpolationVars(enValue)
    const itVars = extractInterpolationVars(itValue)

    for (const v of enVars) {
      if (localeSpecificVars.has(v)) continue
      if (!itVars.has(v)) {
        errors.push(`${namespace}.${key}: missing {{${v}}} in it`)
      }
    }
  }
...
function checkHtmlTags(enFlat, itFlat, namespace) {
  const errors = []
  ...
  for (const [key, enValue] of enFlat) {
    ...
    const itTags = extractHtmlTags(itValue).sort()

    if (enTags.join(",") !== itTags.join(",")) {
      const missing = enTags.filter((t) => !itTags.includes(t))
      const extra = itTags.filter((t) => !enTags.includes(t))
      const parts = []
...
function checkEmptyStrings(itFlat, namespace) {
  const errors = []

  for (const [key, value] of itFlat) {
    if (value === "") {
      errors.push(`${namespace}.${key}`)
    }
  }
...
function checkUntranslated(enFlat, itFlat, namespace) {
  const explained = []
  const unexplained = []
...
if (!fs.existsSync(itRoot)) {
  console.log("it locale directory not found, skipping coverage check.")
  process.exit(0)
}
...
for (const ns of namespaces) {
  ...
  const itPath = path.join(itRoot, `${ns}.json`)
  ...
  if (!fs.existsSync(itPath)) {
    errors.missingKeys.push(`${ns}: it locale file missing`)
    continue
  }
  ...
  try {
    ...
    itData = JSON.parse(fs.readFileSync(itPath, "utf8"))
  } catch {
  ...
  const itFlat = flattenKeys(itData)

  errors.missingKeys.push(...checkMissingKeys(enFlat, itFlat, ns))
  errors.extraKeys.push(...checkExtraKeys(enFlat, itFlat, ns))
  errors.interpolation.push(...checkInterpolation(enFlat, itFlat, ns))
  errors.htmlTags.push(...checkHtmlTags(enFlat, itFlat, ns))
  errors.emptyStrings.push(...checkEmptyStrings(itFlat, ns))

  const untranslated = checkUntranslated(enFlat, itFlat, ns)
...
if (totalErrors === 0 && totalWarnings === 0) {
  console.log("it translation coverage: all checks passed.")
  process.exit(0)
}

console.log("=== it Translation Coverage Report ===\n")
