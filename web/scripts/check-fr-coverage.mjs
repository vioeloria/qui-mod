...
const frRoot = path.join(localesRoot, "fr")
...
function checkMissingKeys(enFlat, frFlat, namespace) {
  const errors = []

  // Build set of en plural bases that have _one/_other pairs (v4 CLDR style).
  // For these, fr only needs _other -- _one is not required.
  ...
  for (const [key, value] of enFlat) {
    // Skip _one keys for v4 plural pairs -- fr collapses to _other only.
...
function checkExtraKeys(enFlat, frFlat, namespace) {
  const errors = []

  for (const key of frFlat.keys()) {
    if (!enFlat.has(key)) {
      // If fr has _other and en has _one/_other pair, that is fine.
...
function checkInterpolation(enFlat, frFlat, namespace) {
  const errors = []
  ...
  for (const [key, enValue] of enFlat) {
    ...
    const frVars = extractInterpolationVars(frValue)

    for (const v of enVars) {
      if (localeSpecificVars.has(v)) continue
      if (!frVars.has(v)) {
        errors.push(`${namespace}.${key}: missing {{${v}}} in fr`)
      }
    }
  }
...
function checkHtmlTags(enFlat, frFlat, namespace) {
  const errors = []
  ...
  for (const [key, enValue] of enFlat) {
    ...
    const frTags = extractHtmlTags(frValue).sort()

    if (enTags.join(",") !== frTags.join(",")) {
      const missing = enTags.filter((t) => !frTags.includes(t))
      const extra = frTags.filter((t) => !enTags.includes(t))
      const parts = []
...
function checkEmptyStrings(frFlat, namespace) {
  const errors = []
...
function checkUntranslated(enFlat, frFlat, namespace) {
  const explained = []
  const unexplained = []
...
if (!fs.existsSync(frRoot)) {
  console.log("fr locale directory not found, skipping coverage check.")
  process.exit(0)
}
...
for (const ns of namespaces) {
  ...
  const frPath = path.join(frRoot, `${ns}.json`)
  ...
  if (!fs.existsSync(frPath)) {
    errors.missingKeys.push(`${ns}: fr locale file missing`)
    continue
  }

  errors.encoding.push(...checkEncoding(frPath))
  ...
  errors.missingKeys.push(...checkMissingKeys(enFlat, frFlat, ns))
  errors.extraKeys.push(...checkExtraKeys(enFlat, frFlat, ns))
  errors.interpolation.push(...checkInterpolation(enFlat, frFlat, ns))
  errors.htmlTags.push(...checkHtmlTags(enFlat, frFlat, ns))
  errors.emptyStrings.push(...checkEmptyStrings(frFlat, ns))

  const untranslated = checkUntranslated(enFlat, frFlat, ns)
...
if (totalErrors === 0 && totalWarnings === 0) {
  console.log("fr translation coverage: all checks passed.")
  process.exit(0)
}

console.log("=== fr Translation Coverage Report ===\n")
