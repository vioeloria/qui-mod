...
const koRoot = path.join(localesRoot, "ko")
...
function checkMissingKeys(enFlat, koFlat, namespace) {
  const errors = []

  // Build set of en plural bases that have _one/_other pairs (v4 CLDR style).
  // For these, ko only needs _other -- _one is not required.
  ...
  for (const [key, value] of enFlat) {
    // Skip _one keys for v4 plural pairs -- ko collapses to _other only.
...
function checkExtraKeys(enFlat, koFlat, namespace) {
  const errors = []

  for (const key of koFlat.keys()) {
    if (!enFlat.has(key)) {
      // If ko has _other and en has _one/_other pair, that is fine.
...
function checkInterpolation(enFlat, koFlat, namespace) {
  const errors = []

  for (const [key, enValue] of enFlat) {
    const koValue = koFlat.get(key)
    if (!koValue) continue
    ...
    for (const v of enVars) {
      ...
      if (!koVars.has(v)) {
        errors.push(`${namespace}.${key}: missing {{${v}}} in ko`)
      }
    }
  }
...
function checkHtmlTags(enFlat, koFlat, namespace) {
  const errors = []
  ...
  for (const [key, enValue] of enFlat) {
    ...
    const koTags = extractHtmlTags(koValue).sort()

    if (enTags.join(",") !== koTags.join(",")) {
      const missing = enTags.filter((t) => !koTags.includes(t))
      const extra = koTags.filter((t) => !enTags.includes(t))
      const parts = []
...
function checkEmptyStrings(koFlat, namespace) {
  const errors = []
...
function checkUntranslated(enFlat, koFlat, namespace) {
  const explained = []
  const unexplained = []
...
if (!fs.existsSync(koRoot)) {
  console.log("ko locale directory not found, skipping coverage check.")
  process.exit(0)
}
...
for (const ns of namespaces) {
  ...
  const koPath = path.join(koRoot, `${ns}.json`)
  ...
  if (!fs.existsSync(koPath)) {
    errors.missingKeys.push(`${ns}: ko locale file missing`)
    continue
  }

  errors.encoding.push(...checkEncoding(koPath))
  ...
  errors.missingKeys.push(...checkMissingKeys(enFlat, koFlat, ns))
  errors.extraKeys.push(...checkExtraKeys(enFlat, koFlat, ns))
  errors.interpolation.push(...checkInterpolation(enFlat, koFlat, ns))
  errors.htmlTags.push(...checkHtmlTags(enFlat, koFlat, ns))
  errors.emptyStrings.push(...checkEmptyStrings(koFlat, ns))

  const untranslated = checkUntranslated(enFlat, koFlat, ns)
...
if (totalErrors === 0 && totalWarnings === 0) {
  console.log("ko translation coverage: all checks passed.")
  process.exit(0)
}

console.log("=== ko Translation Coverage Report ===\n")
