...
const csRoot = path.join(localesRoot, "cs")
...
function checkInterpolation(enFlat, csFlat, namespace) {
  const errors = []

  for (const [key, enValue] of enFlat) {
    const csValue = csFlat.get(key)
    if (!csValue) continue

    const enVars = extractInterpolationVars(enValue)
    const deVars = extractInterpolationVars(csValue)

    for (const v of enVars) {
      if (localeSpecificVars.has(v)) continue
      if (!deVars.has(v)) {
        errors.push(`${namespace}.${key}: missing {{${v}}} in cs`)
      }
    }
  }
...
function checkHtmlTags(enFlat, csFlat, namespace) {
  const errors = []
  ...
  for (const [key, enValue] of enFlat) {
    ...
    const deTags = extractHtmlTags(csValue).sort()
...
function checkEmptyStrings(csFlat, namespace) {
  const errors = []

  for (const [key, value] of csFlat) {
    if (value === "") {
      errors.push(`${namespace}.${key}`)
    }
  }
...
for (const ns of namespaces) {
  ...
  const csPath = path.join(csRoot, `${ns}.json`)
  ...
  if (!fs.existsSync(csPath)) {
    errors.missingKeys.push(`${ns}: cs locale file missing`)
    continue
  }

  errors.encoding.push(...checkEncoding(csPath))

  let enData, csData
  try {
    enData = JSON.parse(fs.readFileSync(enPath, "utf8"))
    csData = JSON.parse(fs.readFileSync(csPath, "utf8"))
  } catch {
  ...
  const csFlat = flattenKeys(csData)
  ...
  const untranslated = checkUntranslated(enFlat, csFlat, ns)
...
if (totalErrors === 0 && totalWarnings === 0) {
  console.log("cs translation coverage: all checks passed.")
  process.exit(0)
}

console.log("=== cs Translation Coverage Report ===\n")
