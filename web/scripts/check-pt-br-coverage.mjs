...
const ptBrRoot = path.join(localesRoot, "pt-BR")
...
// to leave untranslated in pt-BR.
...
function checkExtraKeys(enFlat, ptBrFlat, namespace) {
  const errors = []
...
function checkInterpolation(enFlat, ptBrFlat, namespace) {
  const errors = []

  for (const [key, enValue] of enFlat) {
    const ptBrValue = ptBrFlat.get(key)
    if (!ptBrValue) continue
    ...
    for (const v of enVars) {
      ...
      if (!ptBrVars.has(v)) {
        errors.push(`${namespace}.${key}: missing {{${v}}} in pt-BR`)
      }
    }
  }
...
function checkHtmlTags(enFlat, ptBrFlat, namespace) {
  const errors = []
  ...
  for (const [key, enValue] of enFlat) {
    ...
    const ptBrTags = extractHtmlTags(ptBrValue).sort()

    if (enTags.join(",") !== ptBrTags.join(",")) {
      const missing = enTags.filter((t) => !ptBrTags.includes(t))
      const extra = ptBrTags.filter((t) => !enTags.includes(t))
      const parts = []
...
function checkEmptyStrings(ptBrFlat, namespace) {
  const errors = []

  for (const [key, value] of ptBrFlat) {
    if (value === "") {
      errors.push(`${namespace}.${key}`)
    }
  }
...
function checkUntranslated(enFlat, ptBrFlat, namespace) {
  const explained = []
  const unexplained = []
...
if (!fs.existsSync(ptBrRoot)) {
  console.log("pt-BR locale directory not found, skipping coverage check.")
  process.exit(0)
}
...
for (const ns of namespaces) {
  ...
  const ptBrFlat = flattenKeys(ptBrData)

  errors.missingKeys.push(...checkMissingKeys(enFlat, ptBrFlat, ns))
  errors.extraKeys.push(...checkExtraKeys(enFlat, ptBrFlat, ns))
  errors.interpolation.push(...checkInterpolation(enFlat, ptBrFlat, ns))
  errors.htmlTags.push(...checkHtmlTags(enFlat, ptBrFlat, ns))
  errors.emptyStrings.push(...checkEmptyStrings(ptBrFlat, ns))

  const untranslated = checkUntranslated(enFlat, ptBrFlat, ns)
...
if (totalErrors === 0 && totalWarnings === 0) {
  console.log("pt-BR translation coverage: all checks passed.")
  process.exit(0)
}

console.log("=== pt-BR Translation Coverage Report ===\n")
